...
export function CourseStructure({ data }: iAppProps) {
  const initialItems: ChapterItem[] =
    data.chapter.map((chapter: AdminCourseSingularType["chapter"][number]) => ({
      id: chapter.id,
      title: chapter.title,
      order: chapter.position,
      isOpen: true,
      lessons: chapter.lessons.map((lesson: typeof chapter.lessons[number]) => ({
        id: lesson.id,
        title: lesson.title,
  ...
  useEffect(() => {
    setItems((prevItems: ChapterItem[]) => {
      const updatedItems: ChapterItem[] =
        data.chapter.map((chapter: AdminCourseSingularType["chapter"][number]) => ({
          id: chapter.id,
          title: chapter.title,
          order: chapter.position,
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (activeType === "chapter") {
      ...
      if (oldIndex === -1 || newIndex === -1) {
        toast.error("Could not find chapter old/new for ordering");

        return;
      }

      const reoderLocalChapter = arrayMove(items, oldIndex, newIndex);

      const updatedChapterForState = reoderLocalChapter.map(
        (chapter, index) => ({
          ...chapter,
          order: index + 1,
        })
    ...
    if (activeType === "lesson" && overType === "lesson") {
      ...
      if (courseId) {
        ...
        toast.promise(reoderLessonsPromise(), {
          ...
          error: () => {
            setItems(previousItems);
            return "Failed to reorder lessons. Changes reverted.";
          },
        });
      }
