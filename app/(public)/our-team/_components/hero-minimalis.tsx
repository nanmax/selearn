  ...
  return (
    <>
      <section
        ...
        bg-linear-to-b from-green-50 via-green-100/40 to-white 
        ...
        <motion.div
          ...
          className="flex items-center justify-center gap-4 flex-col">
          <motion.h1
            className="text-6xl max-sm:text-4xl font-medium tracking-tighter"
            initial={{ scale: 0.95, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ duration: 0.6, delay: 0.3, ease: "easeOut" }}>
            Di Balik Layar Selearn
          </motion.h1>
          <motion.p
            className="max-sm:text-sm text-gray-500 text-balance"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.6, delay: 0.4 }}>
            Kami adalah para pendidik, inovator, dan kreator yang disatukan oleh
            satu visi: menjadikan pendidikan berkualitas dapat diakses oleh
            seluruh Indonesia, dimulai dari tanah kelahiran kami.
          </motion.p>
        </motion.div>
        <motion.div
          ...
          <Button asChild className="grow rounded-xl" size={"sm"}>
            <Link
              ...
              Mulai Sekarang <span className="opacity-70">- Gratis!</span>
            </Link>
          </Button>
        </motion.div>
        <motion.div
          ...
          className="border border-gray-200 dark:border-gray-800 shadow-lg rounded-3xl overflow-hidden w-full">
          <VideoPlayer
...
const VideoPlayer = React.forwardRef<HTMLVideoElement, VideoPlayerProps>(
  (
    ...
    const skip = (seconds: number) => {
      ...
      if (!v) return;
      const target =
        (hasFiniteDuration ? currentTime : v.currentTime) + seconds;
      const clamped = hasFiniteDuration
      ...
      <div
        ...
        {showControls && (
          <>
            ...
            <div
              className={cn(
                "absolute bottom-0 start-0 end-0 bg-linear-to-t from-black/80 to-transparent",
